...
class FaceBoundsOverlay @JvmOverloads constructor(ctx: Context, attrs: AttributeSet? = null) :
    ...
    init {
        anchorPaint.color = ContextCompat.getColor(context, R.color.colorPrimary)

        idPaint.color = ContextCompat.getColor(context, R.color.colorPrimary)
        idPaint.textSize = 40f

        boundsPaint.style = Paint.Style.STROKE
        boundsPaint.color = ContextCompat.getColor(context, R.color.colorPrimary)
